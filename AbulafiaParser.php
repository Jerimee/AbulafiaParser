...
function wfRandomExtension() {
    ...
    $wgParser->setHook( "sgdisplay", "renderRandom" );
}

function tableslicer(){
	$wgHooks['ArticleSave'][] = 'parseForDb' ;
}

function parseForDb($input, $argv, $parser ){
		//get db
	$db =& wfGetDB( DB_SLAVE );
        $parser->disableCache();	
	$ret='';
	...
	$deletesql = 'DELETE FROM abulafia_entries2 WHERE sgtable=\''.mysql_escape_string($tablename).'\'';
	...
	foreach($subtables as $subtable){
		...
		$query = 'INSERT INTO abulafia_entries2 values (DEFAULT,\''.mysql_escape_string($tablename).'\',\''.mysql_escape_string($subtablename).'\',\''.mysql_escape_string($serializedrelarray).'\')';
...
function callFromDb($tablename,$page,$calldepth){
	//get db
	$db =& wfGetDB( DB_SLAVE );
	...
	$sql = 'SELECT * FROM abulafia_entries2 WHERE sgtable = \''.mysql_escape_string($tablename).'\' AND subtable = \''.mysql_escape_string($subtablename).'\'';
	...
	if (sizeof($randtable)>0){
	    ...
	    if ($calldepth<150){
		$output .= sgProcessEntry($randtable[$choice],$page,$calldepth);
...
function sgProcessEntry($incoming,$page,$calldepth){
	...
	if (strpos($incoming,'[') === false)//no tables, feed on through
	{
		...
		while($done == false){
			...
			if ($closebracket === false){//tablename never terminates
				...
				$output .= callFromDb($tablename,$page,$calldepth+1);
...
function renderRandom( $input, $argv, $parser ) {
	...
	if((0 < $iterations)&&($iterations < 100)){
		for($i=0;$i<$iterations;$i++){
			...
			$outputstr.=sgProcessEntry($input,$parser->mTitle->getText(),0);
